...
Além disso, o documento será utilizado como referência no planejamento, implementação, testes e manutenção do sistema, para assim garantir qur todas as necessidades referentes ai sistema sejam satisfeitas.

O público-alvo deste documentoenvolve:
...
Seção 4: Acrescenta ao documento informações complementare que auxiliam o entendimento dos requisitos do sistema e do funcionamento geral do sistema.
...
O Sistema de Recursos será desenvolvido como uma aplicação independente. Ele tem como objetivo auxiliar na organização e gerenciamento do estoque e vendar, como também acompanhar o desempenho financeiro da empresa.
...
RG04 - O sistema deve ser executável em qualquer sistema operacional compatível com a JVM (Java Virtual Machine), incluindo Windows e Linux.

RG05 - O sistema operará em modo de usuário único por sessão, sem suporte a acesso simultâneo de múltiplos usuários em tempo real.

RG06 - O acesso ao sistema deve restrito a usuários autenticados por meio de login e senha. Nenhuma funcionalidade do sistema pode ser acessada sem autenticação prévia.
